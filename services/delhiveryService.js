 ...
 */
export async function getRates({ md, cgm, o_pin, d_pin, ss = 'Delivered' }) {
...
/**
 * Get shipping label PDF for one or more waybills.
 * @param {string|string[]} waybills
 * @returns Buffer containing the PDF
 */
export async function getLabel(waybills) {
  const wbns = Array.isArray(waybills) ? waybills.join(',') : waybills;
  const response = await client.get('/api/p/packing_slip', {
    params: { wbns, token: process.env.DELHIVERY_TOKEN },
